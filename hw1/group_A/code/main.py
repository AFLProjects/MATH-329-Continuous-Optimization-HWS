...
test_X, test_Y = np.asarray(test.X), np.asarray(test.y)
...
def run_gradient_descent(theta_0, alpha = 1e-3, time_limit = 3.0 * 60):
    theta = theta_0

    grad_0 = grad_f(theta_0)
    grad = grad_f(theta)

    values_f = [f(theta_0)]
    values_grad_f_norm = [np.linalg.norm(grad_0)]
